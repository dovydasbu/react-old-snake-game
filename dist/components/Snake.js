...
class Snake extends Component {
  ...
  componentDidMount() {
    this._isMounted = true; // Start moving the snake

    this.setMovingInterval(this.state.speed); // Manage arrow keys gameplay, because react-key-handler does not work properly
  ...
  foodEaten() {
    ...
    let {
      squares
    } = this.state; // Change food position

    onFoodEaten(squares); // Add square to snake
  ...
  moveSnake() {
    ...
    let {
      ...
      onGameOver,
      isPause
    } = this.props; // Check if head has hit another square or went out of playing area
    ...
    if (this.hasAteItself(squares) || this.isOutOfBounds(squares)) {
      ...
      squares.map((square, index) => {
        // Move current square
        square = this.moveSquare(square); // Set square direction

        square = this.changeSquareDirection(squares, square, index); // Check if head has reached a food
  ...
  changeSquareDirection(squares, square, index) {
    ...
    if (square.head !== undefined && square.head && keyStack.length > 0) {
      ...
      if (square.direction === 'up' && keyStack[0] !== 'down' || square.direction === 'right' && keyStack[0] !== 'left' || square.direction === 'down' && keyStack[0] !== 'up' || square.direction === 'left' && keyStack[0] !== 'right') {
  ...
  isOutOfBounds(squares) {
    ...
    if (head.x <= 0 && head.direction === 'left' || head.x >= areaParams.width - squareSize && head.direction === 'right' || head.y <= 0 && head.direction === 'up' || head.y >= areaParams.height - squareSize && head.direction === 'down') {
      return true;
    }
  ...
  render() {
    ...
    return /*#__PURE__*/React.createElement(Fragment, null, /*#__PURE__*/React.createElement(KeyHandler, {
      keyEventName: KEYPRESS,
      keyValue: "w",
      onKeyHandle: this.directionToUp
    }), /*#__PURE__*/React.createElement(KeyHandler, {
      keyEventName: KEYPRESS,
      keyValue: "d",
      onKeyHandle: this.directionToRight
    }), /*#__PURE__*/React.createElement(KeyHandler, {
      keyEventName: KEYPRESS,
      keyValue: "s",
      onKeyHandle: this.directionToDown
    }), /*#__PURE__*/React.createElement(KeyHandler, {
      keyEventName: KEYPRESS,
      keyValue: "a",
      onKeyHandle: this.directionToLeft
    }), squares !== undefined && squares.length > 0 && squares.map((square, key) => /*#__PURE__*/React.createElement(Square, _extends({
      key: key
    }, square))));
  }
